...
## 十、Debugging Workflow（调试工作流）

当遇到 UI 渲染、API 响应、或任何需要前后端协同的问题时，严格遵循以下 4 步流程：

### Step 1: Playwright + Logs 确认问题

**目标：** 通过自动化测试和实时日志，客观确认问题现象，而非主观猜测。

- **前端问题：** 编写/运行 Playwright E2E 测试，截图并断言 DOM 元素状态
- **后端问题：** 通过 `curl` 或 API 测试捕获响应数据
- **日志监控：** 同时监控前端（Next.js）和后端（API server）日志
  - 后端 stdout/stderr：`logs/server.log`（应用日志、print、uvicorn 默认/错误日志）
  - 后端访问日志：`logs/access.log`（每个 HTTP 请求 + 状态码）
  - 前端 stdout/stderr：`logs/ui.log`（Next.js 请求日志、SSR 报错）
  - 一键实时查看：`tail -f logs/server.log logs/access.log logs/ui.log`

**输出：** 测试失败截图 + 断言错误 + 前后端日志片段

### Step 2: 分析问题原因（前端 or 后端）

**目标：** 根据日志和测试结果，定位根因所在层级。

- **前端问题特征：**
  - DOM 元素存在但样式错误（CSS 问题）
  - DOM 元素不存在（渲染逻辑问题）
  - Console 报错（JavaScript 运行时错误）
  - Network 请求成功但 UI 未更新（状态管理问题）

- **后端问题特征：**
  - Network 请求失败（500/404/403）
  - API 响应数据格式错误
  - API 响应数据缺失或为空
  - 后端日志显示异常（数据库错误、权限错误等）

- **判断依据：**
  - 查看前端日志的 Network 请求状态码
  - 查看后端日志的异常堆栈
  - 对比 API 响应数据与预期数据结构

**输出：** 明确的问题定位（前端/后端/数据层/接口契约）
...
- **原则：** 最小上游修复优于下游绕过
- **前端修复：** 修改组件代码、CSS 样式、状态管理逻辑
- **后端修复：** 修改 API 响应、数据处理逻辑、数据库查询
- **禁止：** 掩盖症状的临时方案（如硬编码数据、忽略错误）
...
1. **客观测试而非主观猜测**
   - Playwright 测试提供了客观的 CSS 属性值（`border: 0px solid` vs `border: 1px solid`）
   - 避免了"看起来像换行"的主观判断，直接定位到"无边框"这个具体问题

2. **前后端日志的价值**
   - 前端日志显示 API 请求成功（200 OK），排除后端数据问题
   - 后端日志无异常，确认数据层和接口契约正常
   - 日志对比快速缩小问题范围：从"前后端都可能有问题"缩小到"仅前端CSS问题"

3. **最小化修复原则**
   - 在正确的位置（前端组件）进行修复
   - 使用 ReactMarkdown 的自定义组件，而非全局 CSS 覆盖
   - 一次修改解决问题，没有引入副作用

4. **自动化验证闭环**
   - 修复后立即运行同一测试，验证 CSS 属性从 `0px` 变为 `1px`
   - 客观数据证明修复有效，无需人工肉眼比对

**关键洞察：**
- 日志是问题的"黑匣子"，记录了系统真实行为
- 测试是问题的"显微镜"，放大细节供分析
- 两者结合，可以从"猜测"升级为"诊断"

---

## 十一、三重反馈（Triple Feedback）：前端 + 后端 + Playwright

Debugging Workflow（章节十）解决的是"怎么查"，本章解决的是"用什么信号查"。Flow Coding 的自愈闭环之所以能可靠收敛，根本在于 AI 拿到的不是单一信号，而是**三路相互独立、可交叉验证的实时反馈**。
...
| ② 前端反馈 | `logs/ui.log`（Next.js stdout + 请求状态码 + SSR 报错） | 前端**做了**什么、请求**发出**了吗？ | 前端运行层 |
| ③ 后端反馈 | `logs/server.log` + `logs/access.log`（应用日志 + uvicorn + 每个 HTTP 请求） | 后端**收到**请求了吗、**返回**了什么、**报错**了吗？ | 后端运行层 |
...
- **后端**：在应用入口 tee `stdout`/`stderr` 到 `logs/server.log`；uvicorn 访问日志单独写 `logs/access.log`。
- **前端**：`dev` 脚本将 `next dev` 输出 `tee` 到 `logs/ui.log`。
