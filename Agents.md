...
- Code reference: `WORKSPACE_DIR = Path.home() / "Downloads" / "safe_claw_worksapce" / "workspace"` in `api/main.py`
...
1. **Create Run/Debug Configuration:**
   ...
   - **Parameters:** `run app.py --server.port 8502 --server.headless false --logger.level debug`
